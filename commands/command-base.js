//Credits to Ubaid7 For making this
...
module.exports = (client, commandOptions) => {
  ...
  client.on('message', (message) => {
    ...
    const { def_prefix } = require('../config.json') // For Prefix
    let prefix = db.fetch(`prefix_${message.guild.id}`) // Get Prefix If Its Set
    if(prefix === null) prefix = db.set(`prefix_${message.guild.id}`, def_prefix) // Else Keep As Default Prefix
